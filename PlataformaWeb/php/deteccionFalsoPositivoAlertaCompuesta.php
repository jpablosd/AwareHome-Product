...
function verAlerta(){
    ...
    if (mysql_num_rows($result1) > 0) {
        while ($row1 = mysql_fetch_array($result1)) {
            $id_alerta = $row1["id_alerta_compuesta"];
...
function falsoPositivo($id_alerta){
    $query1 = "SELECT id_falso_positivo_alerta_compuesta, repeticion, tiempo, contador, estado, alerta_compuesta_id_alerta_compuesta FROM falso_positivo_alerta_compuesta WHERE alerta_compuesta_id_alerta_compuesta = '$id_alerta'";
    $result1 = mysql_query($query1) or die(mysql_error());
    if (mysql_num_rows($result1) > 0) {
        while ($row1 = mysql_fetch_array($result1)) {
            ...
            $id_alerta = $row1["alerta_compuesta_id_alerta_compuesta"];
            $repeticion = $row1["repeticion"];
            $tiempo = $row1["tiempo"];
            $id_falso_positivo = $row1["id_falso_positivo_alerta_compuesta"];
            /*
...
function verHistorial($id_alerta, $repeticion, $tiempo, $id_falso_positivo){
    ...
    $query1 = "SELECT fecha, estado_alerta_compuesta FROM historial_alerta_compuesta WHERE id_alerta_compuesta = '$id_alerta' AND estado_alerta_compuesta = 1 ORDER BY id_historial_alerta_compuesta DESC LIMIT $repeticion1";
    // echo $query1;
...
function actualizarFalsoPositivo($id_falso_positivo, $estado){
    ...
    $query2 = "UPDATE falso_positivo_alerta_compuesta SET estado='$estado' WHERE id_falso_positivo_alerta_compuesta='$id_falso_positivo'";  
