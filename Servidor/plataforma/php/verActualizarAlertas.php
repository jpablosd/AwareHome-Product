...
function verAlertas($usuario_id_usuario){		  
    $response = array();
    $query = "SELECT nombre_alerta, estado, sensor, signo, valor FROM alerta WHERE dispositivo_id_dispositivo = '$usuario_id_usuario'";  
...
function actualizar_alertas(){
    ...
    for ($i = 1; $i <= $id_regla_max; $i++){    
        ...
        $regla = "SELECT id_alerta, dispositivo_id_dispositivo, nombre_alerta, sensor, signo, valor, estado FROM alerta WHERE id_alerta = '$i'";
        $sentencia=mysql_query($regla);
        //echo $regla;
        while($rs=mysql_fetch_array($sentencia,MYSQL_BOTH)){
            $id_regla_basica = $rs['id_alerta'];
            $sensor_temperatura_humedad_id_sensor_temperatura_humedad = $rs['dispositivo_id_dispositivo'];
