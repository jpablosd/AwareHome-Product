            ...
            if($sensor == "temperatura")
            {
                if($simbolo == "<")
                {
                    if($temperatura < $dato)
                    {
                        ...
                        echo "<script>var myDataRef = new Firebase('https://awarehome.firebaseio.com/cambio_estado'); var usersRef = myDataRef.child('temperatura');usersRef.update({id: 1}); alert('hols');</script>";
...
//			echo $temperatura." < entre: ".$dato;
  ...
  echo "<script>var myDataRef = new Firebase('https://awarehome.firebaseio.com/cambio_estado'); var usersRef = myDataRef.child('temperatura');usersRef.update({id: 1});</script>";
...
echo "<script>var myDataRef = new Firebase('https://awarehome.firebaseio.com/cambio_estado');  var usersRef = myDataRef.child('humedad');  usersRef.update({   id: 1});</script>";
...
//		echo "sensor gas";
		if($simbolo == "<")
		{
			if($gas < $dato)
			{
				$sql = "UPDATE alerta SET estado = '1' WHERE id_alerta = '$i'";
				mysql_query($sql);	
echo "<script> var myDataRef = new Firebase('https://awarehome.firebaseio.com/cambio_estado'); var usersRef = myDataRef.child('gas');usersRef.update({id: 1});</script>";
			}
			else
			{
				$sql = "UPDATE alerta SET estado = '0' WHERE id_alerta = '$i'";
