...
function verAlertas($usuario_id_usuario){		  
    ...
    $query = "SELECT nombre_alerta, estado, nombre_sensor, simbolo_alerta, dato FROM alerta WHERE usuario_id_usuario = '$usuario_id_usuario'";  
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            $datos["nombre_sensor"]     = $row["nombre_sensor"];
            $datos["simbolo_alerta"]     = $row["simbolo_alerta"];
            $datos["dato"]     = $row["dato"];
